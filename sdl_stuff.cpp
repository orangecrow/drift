...
SDL_AppResult SDL_AppIterate(void *appstate)
{
	...
	if( camera_mode == 1){
		...
		dst_rect.x = car.pos.x;
		dst_rect.y = car.pos.y;
		...
		SDL_RenderLine(renderer,force2.orgin.x,force2.orgin.y,force2.end.x,force2.end.y);
		SDL_RenderLine(renderer,force1.orgin.x,force1.orgin.y,force1.end.x,force1.end.y);
		SDL_RenderLine(renderer,force3.orgin.x,force3.orgin.y,force3.end.x,force3.end.y);
	}
	if( camera_mode == 2){
		...
		SDL_RenderTextureRotated(renderer, texture, NULL, &dst_rect, angle , &center, SDL_FLIP_NONE);
		SDL_SetRenderDrawColor(renderer, 255, 255, 0, SDL_ALPHA_OPAQUE);
		//SDL_RenderLine(renderer,pointA.x+(WINDOW_WIDTH+texture_width)/2,pointA.y+(WINDOW_HEIGHT+texture_height)/2,pointB.x+(WINDOW_WIDTH+texture_width)/2,pointB.y+(WINDOW_HEIGHT+texture_height)/2);
		//SDL_RenderLine(renderer,pointC.x+(WINDOW_WIDTH+texture_width)/2,pointC.y+(WINDOW_HEIGHT+texture_height)/2,pointD.x+(WINDOW_WIDTH+texture_width)/2,pointD.y+(WINDOW_HEIGHT+texture_height)/2);
	}
